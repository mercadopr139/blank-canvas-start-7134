...
import {
  Radio, AlertTriangle, CheckCircle2, CheckCircle, XCircle, Clock, User, MapPin,
  Download, Calendar, Trash2, DollarSign, Baby,
} from "lucide-react";
import { format, endOfMonth, setDate, subDays } from "date-fns";
...
export default function TransportRunsPay() {
  return (
    <div className="p-4 md:p-6 space-y-4">
      <h1 className="text-xl font-bold text-white">Runs & Pay</h1>
...
function LiveTab() {
  ...
  const fetchRuns = async () => {
    const { data } = await supabase
      ...
      .select(`
        ...
        attendance:transport_attendance(id, status, youth:youth_profiles(id, first_name, last_name, pickup_zone))
      `)
      .eq("status", "in_progress")
      .order("started_at", { ascending: false });

    if (data) setRuns(data as unknown as RunWithDetails[]);
    setLoading(false);
  };
  ...
  // Flag youth picked up but not dropped off
  const flaggedYouth = useMemo(() => {
  ...
  if (loading) return <div className="text-white/40 text-center py-12">Loading...</div>;
  ...
  return (
    <div className="space-y-4">
      ...
      {runs.map((run) => {
        ...
        return (
          <div key={run.id} className="bg-green-500/5 border border-green-500/20 rounded-xl p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
                <span className="text-white font-medium capitalize">{run.run_type}</span>
                <Badge variant="outline" className="text-white/50 border-white/20 text-[10px]">{routeName}</Badge>
              </div>
            ...
            <div className="flex items-center gap-2 text-white/50 text-sm">
              ...
              <span className="text-white/40 text-xs">{run.attendance.length} youth</span>
            </div>
            {run.attendance.length > 0 && (
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
                {run.attendance.map((a) => {
                  ...
                  return (
                    <div key={a.id} className={`flex items-center gap-1.5 px-2 py-1.5 rounded-md text-xs ${
                      isFlagged ? "bg-yellow-500/10 text-yellow-300" : "bg-green-500/10 text-green-300"
                    }`}>
                      {isFlagged && <AlertTriangle className="w-3 h-3" />}
                      <MapPin className="w-3 h-3" />
                      <span className="truncate">{a.youth ? `${a.youth.first_name} ${a.youth.last_name[0]}.` : "Unknown"}</span>
                    </div>
                  );
                })}
              </div>
