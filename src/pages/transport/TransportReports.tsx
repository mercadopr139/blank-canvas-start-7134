...
export default function TransportReports() {
  ...
  const [mainTab, setMainTab] = useState<"reports" | "pay">("reports");
      ...
      <div className="flex gap-2">
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
            mainTab === "reports" ? "bg-[#DC2626] text-white" : "bg-white/5 text-white/50 border border-white/10"
          }`}
        >
          <FileBarChart className="w-4 h-4 inline mr-1.5" />
          Reports
        </button>
        <button
          onClick={() => setMainTab("pay")}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
            mainTab === "pay" ? "bg-green-600 text-white" : "bg-white/5 text-white/50 border border-white/10"
          }`}
        >
          💰 Pay Management
        </button>
