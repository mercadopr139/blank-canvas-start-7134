...
export default function TransportYouth() {
  ...
  const openEdit = (y: YouthProfile) => {
    ...
    setForm({
      ...
      address: y.address || "",
      pickup_zone: y.pickup_zone,
      emergency_contact_name: y.emergency_contact_name || "",
      emergency_contact_phone: y.emergency_contact_phone || "",
      notes: y.notes || "",
      status: y.status,
    });
    setPhotoFile(null);
  ...
  const filtered = youth.filter((y) => {
    ...
    return (
      y.first_name.toLowerCase().includes(q) ||
      y.last_name.toLowerCase().includes(q) ||
      y.pickup_zone.toLowerCase().includes(q)
    );
  });
  ...
  const getPhotoUrl = (url: string | null) => {
    ...
    return `${base}/storage/v1/object/public/youth-photos/${url}`;
  };
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <Button onClick={openAdd} className="bg-[#3B82F6] hover:bg-[#2563EB] text-white gap-2">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-white/40 text-sm flex items-center gap-1 mt-0.5">
                  <MapPin className="w-3 h-3" /> {y.pickup_zone}
                </p>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="bg-[#111827] border-white/10 text-white max-w-md max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Youth Profile" : "Add Youth Profile"}</DialogTitle>
          </DialogHeader>
