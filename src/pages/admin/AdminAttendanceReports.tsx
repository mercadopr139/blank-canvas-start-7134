...
const AdminAttendanceReports = () => {
  ...
  const handleExportPdf = () => {
    ...
    programBreakdown.forEach(([prog, count]) => summary.push([`Program: ${prog}`, `${count} of ${uniqueCount} (${pct(count, uniqueCount)})`]));
                  ...
                  filteredAttendance.map((a, i) => {
                    ...
                    return (
                      <TableRow key={a.id} className="border-white/10">
                        ...
                        <TableCell><span className={`text-xs px-1.5 py-0.5 rounded-full ${a.program_source === 'Smile Lab' ? 'bg-teal-500/15 text-teal-400' : a.program_source === 'Lil Champs Corner' ? 'bg-sky-500/15 text-sky-400' : 'bg-green-500/15 text-green-400'}`}>{a.program_source || 'NLA'}</span></TableCell>
