...
export type ExcursionReportSource = {
  id: string;
  name: string;
  date: string;
  details: string | null; // "few words" / Trip Overview
...
interface Props {
  open: boolean;
  source: ExcursionReportSource | null;
  onClose: () => void;
}
...
const ExcursionReportSheet = ({ open, source, onClose }: Props) => {
  ...
  const tripFacts = (s: ExcursionReportSource) => ({
    name: s.name,
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const { data, error } = await supabase.functions.invoke("excursion-report", {
          body: { mode: "generate", trip: tripFacts(source) },
        });
  ...
  const revise = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("excursion-report", {
        body: { mode: "revise", narrative, instruction, trip: tripFacts(source) },
      });
      if (error) throw error;
      if (data?.error) throw new Error(data.error);
  ...
  const download = async () => {
    ...
    await downloadCornerCoachReportPdf({
      ...
      narrative,
      stats: [
        { label: "Youth Attended", value: String(source.youthCount) },
      ],
      table: roster.length > 0 ? { columns: ["Youth on this trip"], rows: roster.map((n) => [n]) } : null,
    });
  };
      ...
      <DialogContent className="bg-neutral-900 border-white/10 text-white max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <FileText className="w-4 h-4 text-purple-400" /> Grant Report — {source?.name}
          </DialogTitle>
        </DialogHeader>
            ...
            <div className="flex items-end gap-2">
              <div className="flex-1">
                ...
                <Input
                  ...
                  placeholder='e.g. "make it more formal" or "emphasize the youth we reached"'
                  disabled={revising}
...
export default ExcursionReportSheet;
