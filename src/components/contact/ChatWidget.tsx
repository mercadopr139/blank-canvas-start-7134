...
const chatResponses: Record<Exclude<QuickOption, null>, {
  ...
  schedule: {
    ...
    content: <div className="space-y-4">
        ...
        <div>
          ...
          <p className="text-xs italic text-muted-foreground mt-1">*Junior Boxing practice ends 6pm, optional aftercare programming available until 7:15pm </p>
        </div>
  ...
  registration: {
    ...
    content: <div className="space-y-3">
        ...
        <ul className="list-disc list-inside text-sm text-muted-foreground space-y-1 ml-2" style={{
  ...
  location: {
    ...
    content: <div className="space-y-3">
        ...
        marginTop: '1.25rem'
          ...
          <a href="https://www.google.com/maps/search/?api=1&query=1086+Rt+47+South+Rio+Grande+NJ+08242" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground rounded-md font-bold text-sm hover:bg-primary/90 transition-colors">
  ...
  person: {
    ...
    content: <div className="space-y-3">
        ...
        <a href={`mailto:${CHRISSY_EMAIL}?subject=${encodeURIComponent("No Limits Academy Inquiry")}&body=${encodeURIComponent("Hi Coach Chrissy,\n\nI have a question about:\n\nName:\nYouth Age (if applicable):\nPhone (optional):\nQuestion:\n\nThanks!")}`} className="inline-block mt-2 text-primary underline font-bold text-sm break-all">
          {CHRISSY_EMAIL}
        </a>
      </div>
  }
...
const ChatWidget = ({
  ...
  return <div className="flex flex-col h-[350px]">
      <Button variant="ghost" size="sm" onClick={selectedOption ? handleBackToQuickOptions : onBack} className="self-start mb-3 -ml-2" aria-label={selectedOption ? "Back to quick options" : "Back to contact options"}>
        <ArrowLeft className="h-4 w-4 mr-1" />
        {selectedOption ? "Back" : "Contact Options"}
      </Button>

      <ScrollArea className="flex-1 pr-4">
        {!selectedOption ? <div className="space-y-3">
